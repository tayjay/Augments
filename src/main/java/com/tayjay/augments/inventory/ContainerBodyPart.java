...
public class ContainerBodyPart extends Container
{
    ...
    private static final int INV_START = InventoryBodyPart.INV_MAX_SIZE, INV_END = INV_START+26,
    ...
    public ContainerBodyPart(EntityPlayer player, InventoryPlayer inventoryPlayer, InventoryBodyPart inventoryItem)
    {
        ...
        for (i = 0; i < inventoryItem.invSize; ++i)
        {
            ...
            this.addSlotToContainer(new SlotAugment(this.inventory,player, i, 80 + (18 * (int)(i/4)), 8 + (18*(i%4))));
        }
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            else
            {
				...
				*/
                ...
                if (itemstack1.getItem() instanceof IAugment && ((IAugment)itemstack1.getItem()).canAdd(itemstack1,inventory.invItem))
                {
                    // Try to merge into your custom inventory slots
                    // We use 'InventoryItem.INV_SIZE' instead of INV_START just in case
                    // you also add armor or other custom slots
                    if (!this.mergeItemStack(itemstack1, 0, this.inventory.invSize, false))
                    {
                        return null;
                    }
                }
